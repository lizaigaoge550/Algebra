...
from read_data_v2 import read_data
import re
import numpy as np

from expression import generate_expression, generate_dict

def get_leaf_logical(leaf,gDict,key):
    #对这个leaf做type-raising
    nodelist = [Node(None,None,leaf)]
    return type_rasing(gDict,nodelist,key)
            ...
            for i in range(n - length):
                ...
                if i == j:
                    for k in range(len(v[i])):
                        g = get_leaf_logical(v[i][k], gDict,key)
                        dic[i][j] += g
                else:
...
if __name__ == '__main__':
    ...
    #input,sql = read_data('SharkAttackTestDir_v2')
    input,sql = read_data('sql_data')
    run_batch(input,gDict,sql)